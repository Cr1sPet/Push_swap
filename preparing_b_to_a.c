...
void	find_stack_a_elem (t_list **stack_a, t_info_a_to_b *elem_info_a_to_b, int stack_a_size)
{
	...
	{
		if ((((elem_info_a_to_b[0]).index_b - (temp_stack_a)->index) < min) && ((elem_info_a_to_b[0]).index_b - (temp_stack_a)->index) > 0)
		{
	...
	if (stack_a_size - 1 - (elem_info_a_to_b[0]).place_a >= (elem_info_a_to_b[0]).place_a)
	{
...
void	find_min_summary_iterations (t_info_a_to_b	*elems_info_a_to_b, int stack_b_size)
{
	...
	if ((elems_info_a_to_b[0]).iterations_sign_a == (elems_info_a_to_b[0]).iterations_sign_b)
	{
...
void print_elems_info_a_to_b (t_info_a_to_b *elems_info_a_to_b, int stack_b_size)
{
	...
	while (i < stack_b_size)
	{
		printf ("~~~~~\nelems_info_a_to_b[%d]:\nindex_a = %d\nindex_b = %d\nplace_a = %d\nplace_b = %d\nnumber_top_iterations_a = %d\nnumber_top_iterations_b = %d\niterations_sign_a = %d\niterations_sign_b = %d\nmin_summary_iterations = %d\n", i, (elems_info_a_to_b[i]).index_a, (elems_info_a_to_b[i]).index_b, (elems_info_a_to_b[i]).place_a, (elems_info_a_to_b[i]).place_b, (elems_info_a_to_b[i]).number_top_iterations_a, (elems_info_a_to_b[i]).number_top_iterations_b, (elems_info_a_to_b[i]).iterations_sign_a, (elems_info_a_to_b[i]).iterations_sign_b, (elems_info_a_to_b[i]).min_summary_iterations);
